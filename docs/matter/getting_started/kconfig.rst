...
For instructions about how to set Kconfig options, see `Configuring and building an application`_.
...
It uses modules such as Bluetooth® LE, the IPv6 stack (for example `Configuring Thread in the nRF Connect SDK`_), `PSA Crypto API configuration`_, or `Using MCUboot in nRF Connect SDK`_.
