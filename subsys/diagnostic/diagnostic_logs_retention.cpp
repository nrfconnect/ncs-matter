...
CHIP_ERROR DiagnosticLogsRetentionReader::GetLogs(chip::MutableByteSpan &outBuffer, bool &outIsEndOfLog)
{
	...
	if (!mReadInProgress) {
		/* Remember the data begin, the data end and the size, as these may change if writes will occur during
		 * read process. */
...
{
#ifdef CONFIG_MATTER_DIAGNOSTIC_LOGS_REMOVE_CRASH_AFTER_READ
	return mDiagnosticLogsRetention.Clear();
#else
	/* Do nothing. */
	return CHIP_NO_ERROR;
#endif /* CONFIG_MATTER_DIAGNOSTIC_LOGS_REMOVE_CRASH_AFTER_READ */
}
