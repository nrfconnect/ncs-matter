...
Nrf::Matter::IdentifyCluster sIdentifyTemperature(kTemperatureMeasurementEndpointId,
						  sIdentifyDelegateImplWeatherStation, sTimerDelegate, IDENTIFY_TYPE);
...
void AppTask::UpdatePowerSourceClusterState()
{
	...
	const AttributeDirtyState batPercentDirty = batPercentRemainingQuiet.SetValue(
		...
		QuieterReportingAttribute<uint8_t>::GetPredicateForSufficientTimeSinceLastDirty(
			kBatteryReportInterval));
	status = Clusters::PowerSource::Attributes::BatPercentRemaining::Set(
		kPowerSourceEndpointId, batteryPercentage,
		batPercentDirty == AttributeDirtyState::kMustReport ? MarkAttributeDirty::kYes :
								      MarkAttributeDirty::kNo);
	if (status != Protocols::InteractionModel::Status::Success) {
	...
	const AttributeDirtyState batTimeDirty = batTimeRemainingQuiet.SetValue(
		...
		QuieterReportingAttribute<uint32_t>::GetPredicateForSufficientTimeSinceLastDirty(
			kBatteryReportInterval));
